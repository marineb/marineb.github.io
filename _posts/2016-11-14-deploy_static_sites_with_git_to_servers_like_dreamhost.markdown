...
I originally researched this a while back but couldn't figure it out then. Until today, I've been using [Deploybot](https://deploybot.com/) which does the job for you (deploying websites). Pretty great product. They recently got rid of their free service and I didn't want to pay for a task that would *seem* so simple to setup.

> TODO: Ideally, I'd like to make this work via Github's webhooks (any update to a specific branch would trigger a deploy). I want to figure that part out at some point. 
